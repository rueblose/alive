...
{
    /// <summary>
    /// Точка входа приложения: главное окно Alive плюс версии проекта (Forks) и облако
    /// точек по всей библиотеке (Stat).
    ///
    /// Раньше отсюда собирался второй exe, AliveReel, а Alive.exe оставался без Forks и
    /// Stat. Двух сборок больше нет — exe один, и этот Main единственный. Но приём
    /// остался прежним и менять его незачем: оба окна прицепляются к готовому MainForm
    /// снаружи, через открытые события и свойства (DetailPanel.ForksRequested,
    /// RowListView.Selected -> SetEntry.Path), а не правкой самого каталога. Никакой
    /// рефлексии по приватным полям: то, что нельзя спросить публично, тут и не
    /// спрашивается.
    ///
    /// Когда версии переедут внутрь каталога по-настоящему, всё это заменится третьей
    ...
    internal static class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // Пробные копии помощника по восстановлению живут в чужих папках проектов
            // и должны исчезать сразу после пробы. Если прошлый запуск убили на середине,
            // они там остались — убираем до первого сканирования, чтобы каталог их даже
            // не увидел.
