@echo off
setlocal
rem Console harness for the sample library: the walk, the cache, the usage and the AIFF
rem reader. Not shipped - this checks the thing, it is not part of the thing.
rem
rem   build-library-test.cmd [output folder]     default: %TEMP%\alive-library-test
rem
rem ASCII only: cmd.exe reads batch files in the OEM codepage.

set OUT=%~1
if "%OUT%"=="" set OUT=%TEMP%\alive-library-test
