...
`Ctrl ,` или шестерёнка в шапке. Плавная прокрутка, отключение прозрачности (на
Windows 10 акрил пересчитывается на каждый сдвиг окна, и оно едет за курсором с задержкой) и
откуда берётся список плагинов — из базы самой Live или обходом папок VST2/VST3.

**Обновления.** Раз в сутки Alive спрашивает у GitHub, не вышла ли версия свежее, и говорит об
этом точкой на шестерёнке. Ни всплывающих окон, ни диалогов поперёк работы: точка ждёт, пока на
неё посмотрят. Загорается только на релиз, в котором вырос мажор или минор, — про заплатку надо
спросить самому. Кнопка **Check for updates** в той же строке спрашивает сейчас же и сообщает про
любое обновление, заплатки в том числе.

Этот запрос — единственный, который программа вообще делает. Вместе с ним не уходит ничего: ни о
вас, ни о вашей библиотеке, ни о машине. Это обычный GET за публичной страницей, и GitHub узнаёт
из него ровно то же, что любой веб-сервер узнаёт про всякого, кто открыл у него страницу.
Переключатель рядом отключает и это — тогда Alive не трогает сеть никогда.
